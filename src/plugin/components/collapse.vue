<template>
  <div class="admin-collapse">
    <div class="box-content">
      ...
      <el-button v-if="showCtrl && (!$slots.trigger) && (!$scopedSlots.trigger)" :icon="`el-icon-arrow-${collapsed ? 'down' : 'up'}`" circle @click="collapsed = !collapsed"></el-button>
    </div>
    <div class="box-collapsed" :class="[collapsedContentClass]" v-show="!collapsed">
      <slot name="collapsed" />
    </div>
    <span @click="collapsed = !collapsed" v-if="showCtrl">
