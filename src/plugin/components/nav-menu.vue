...
export default class NavMenu extends Vue {
  ...
  renderSubMenu(route, index) {
    ...
    return route.meta && children.length ?
      <el-submenu key={route.name} index={String(index)}>
        <template slot="title"><div><i class={this.getIcon(route)}></i>{this.getTitle(route)}</div></template>
  ...
  render() {
    return (
      <el-menu ref="menu" default-active={this.active} props={this.menu} onSelect={this.handleSelect}>
